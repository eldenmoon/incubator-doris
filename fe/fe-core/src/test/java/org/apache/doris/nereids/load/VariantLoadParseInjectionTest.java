...
public class VariantLoadParseInjectionTest extends TestWithFeService {
    ...
    public void testStreamLoadParseInjection() throws Exception {
        for (InputShape shape : InputShape.values()) {
            assertParseInjection(createStreamFixture(shape), shape);
        }
    }
    ...
    public void testBrokerLoadParseInjection() throws Exception {
        for (InputShape shape : InputShape.values()) {
            assertParseInjection(createBrokerFixture(shape), shape);
        }
    }
    ...
    public void testRoutineLoadParseInjection() throws Exception {
        for (InputShape shape : InputShape.values()) {
            assertParseInjection(createRoutineFixture(shape), shape);
        }
    }

    private void assertParseInjection(LoadFixture fixture, InputShape shape) {
        ...
        VariantType targetType = targetVariantType(sink);
        List<ParseToVariant> parses = new ArrayList<>();
        List<Cast> stringToVariantCasts = new ArrayList<>();
        for (Plan node : ((Plan) sink.child()).<Plan>collectToList(ignored -> true)) {
            for (Expression expression : node.getExpressions()) {
                parses.addAll(expression.<ParseToVariant>collectToList(ParseToVariant.class::isInstance));
        ...
        Assertions.assertEquals(targetType, parses.get(0).getDataType(), fixture.evidence());
        VariantType parseType = (VariantType) parses.get(0).getDataType();
        Assertions.assertEquals(targetType.toSql(), parseType.toSql(), fixture.evidence());
        Assertions.assertEquals(2048, parseType.getVariantMaxSubcolumnsCount(), fixture.evidence());
        Assertions.assertEquals(321, parseType.getVariantMaxSparseColumnStatisticsSize(), fixture.evidence());
        Assertions.assertEquals(4, parseType.getVariantSparseHashShardCount(), fixture.evidence());
