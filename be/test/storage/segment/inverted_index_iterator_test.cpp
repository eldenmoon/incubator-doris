...
protected:
    ...
    // A values index answering a whole-document predicate: exact rows, no residual.
    std::shared_ptr<MockInvertedIndexReader> create_mock_values_reader(const std::string& parser,
                                                                       InvertedIndexReaderType type,
                                                                       int64_t index_id) {
        std::map<std::string, std::string> properties = {
                {INVERTED_INDEX_PARSER_KEY, parser},
                {std::string(variant_root_index::VARIANT_INDEX_SCOPE_KEY),
                 std::string(variant_root_index::VARIANT_INDEX_SCOPE_VALUES)},
...
// A VARIANT column may carry an exact and an analyzed values index: equality selects the exact
// one, MATCH the analyzed one, and an explicit analyzer identity picks among analyzed indexes.
TEST_F(InvertedIndexIteratorTest, VariantValuesSelectsExactForEqualityAndTokenForMatch) {
    InvertedIndexIterator iterator;
    auto exact_reader = create_mock_values_reader(INVERTED_INDEX_PARSER_NONE,
                                                  InvertedIndexReaderType::STRING_TYPE, 100);
    auto token_reader = create_mock_values_reader(INVERTED_INDEX_PARSER_ENGLISH,
...
TEST_F(InvertedIndexIteratorTest, VariantValuesMatchUsesExplicitAnalyzerIdentity) {
    InvertedIndexIterator iterator;
    auto english = create_mock_values_reader(INVERTED_INDEX_PARSER_ENGLISH,
                                             InvertedIndexReaderType::FULLTEXT, 20);
    auto chinese = create_mock_values_reader(INVERTED_INDEX_PARSER_CHINESE,
