...
TEST_F(IndexCompactionTest, snii_native_merge_validates_rowids_once_and_matches_raw_rebuild) {
    ...
    // Non-zero start ids make destination segment ids differ from their ordinals, which is what
    // do_inverted_index_compaction() must index the destination writers by.
    constexpr int32_t output_segment_start_id = 10;
