...
TEST(VariantTermCodecTest, GoldenLayoutIsTagValueSeparatorPath) {
    ...
    // The marker of an unspellable scalar is the tag alone.
    EXPECT_EQ(root_prefix_other(), bytes({0x08}));
    EXPECT_EQ(decode(root_prefix_other())->tag, Tag::OTHER);
...
TEST(VariantTermCodecTest, DecodeRejectsMalformedTerms) {
    ...
    EXPECT_FALSE(decode(bytes({0x09, 1, 2})).has_value());    // unknown tag
