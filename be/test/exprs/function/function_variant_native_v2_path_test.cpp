...
ColumnVariantV2::MutablePtr raw_column(StringRef metadata, StringRef value) {
    auto result = ColumnVariantV2::create();
    insert_encoded_field(*result, encode_json("null"));
    auto metadatas = ColumnString::create();
    metadatas->insert_data(metadata.data, metadata.size);
    ColumnVariantV2::TestAccess::replace_encoded_subcolumn(*result, 0, metadatas->get_ptr());
    auto values = ColumnString::create();
...
TEST(VariantNativeV2PathTest, TypedStringAndOtherScalarRootsAreNotContainers) {
...
TEST(VariantNativeV2PathTest, BadInputIsInvalidArgumentAndOutputIsAtomic) {
    ...
    std::string trailing_value(valid.ref().value.data, valid.ref().value.size);
