...
static void build_all_row_store_types_block(Block& block, TabletSchema& schema) {
    ...
    // ---- VARIANT (parsed from JSON literals) ----
    {
        ...
        slices.reserve(std::size(jsons));
        for (const auto* j : jsons) {
            slices.emplace_back(j, strlen(j));
        }
        DataTypeVariantV2SerDe serde;
        DataTypeSerDe::FormatOptions options;
        uint64_t num_deserialized = 0;
        THROW_IF_ERROR(serde.deserialize_column_from_json_vector(*col, slices, &num_deserialized,
                                                                 options));
        CHECK_EQ(num_deserialized, std::size(jsons));
...
TEST(BlockSerializeTest, AllRowStoreSupportedTypes) {
    ...
    {
        // c_variant: the row-store JSONB round-trip reconstructs the V2 column directly.
        ...
        for (size_t i = 0; i < 3; ++i) {
            ...
            ASSERT_TRUE(serde.serialize_one_cell_to_json(col, static_cast<int64_t>(i), writer, opts)
                                .ok());
            writer.commit();
            const std::string out = output->get_data_at(0).to_string();
