...
ColumnVariantV2::MutablePtr trailing_value_column() {
    ...
    constexpr std::array<char, 1> valid_value {0};
    constexpr std::array<uint32_t, 2> value_offsets {0, 1};
    auto result = ColumnVariantV2::create();
    result->insert_encoded_rows({.metadata_bytes = {metadata.data(), metadata.size()},
                                 .metadata_offsets = metadata_offsets,
                                 .meta_ids = ids,
                                 .value_bytes = {valid_value.data(), valid_value.size()},
                                 .value_offsets = value_offsets});
    auto malformed_values = ColumnString::create();
    constexpr std::array<char, 2> trailing_value {0, 0};
    malformed_values->insert_data(trailing_value.data(), trailing_value.size());
    ColumnVariantV2::TestAccess::replace_encoded_subcolumn(*result, 2, malformed_values->get_ptr());
    return result;
}
