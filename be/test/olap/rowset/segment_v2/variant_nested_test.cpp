...
TEST_F(VariantNestedTest, test_compaction_openclaw_with_id_10_segments) {
    ...
    // The upstream openclaw sample may contain one malformed giant JSON row.
    // Keep the test deterministic by replacing only the broken row with a
    // minimal valid event payload that exercises the same nested paths.
    {
        rapidjson::Document parsed;
        if (parsed.Parse(json_rows.back().data(), json_rows.back().size()).HasParseError()) {
            json_rows.back() = R"({"session_id":10,"events":[{"type":"agent.start","seq":1,"params":{"query":"fallback-start"}},{"type":"message.in","seq":2,"params":{"query":"fallback-query"}}]})";
        }
    }

    std::vector<RowsetSharedPtr> input_rowsets;
    input_rowsets.reserve(json_rows.size());
    const bool old_throw_on_invalid_json = config::variant_throw_exeception_on_invalid_json;
    config::variant_throw_exeception_on_invalid_json = true;
    Defer reset_throw_on_invalid_json(
            [&] { config::variant_throw_exeception_on_invalid_json = old_throw_on_invalid_json; });
