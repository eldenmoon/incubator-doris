...
#include "exprs/function/variant_parse.h"
#include "util/jsonb_document.h"
#include "util/jsonb_utils.h"

namespace doris {

Status parse_json_strings_to_variant(const ColumnString& strings,
                                     const NullMap::value_type* input_null_map, bool input_is_jsonb,
                                     bool error_to_null, VariantParseResult* result) {
    if (result == nullptr) {
        return Status::InvalidArgument("Variant parse result must not be null");
    }

    auto result_nulls = ColumnUInt8::create(strings.size(), uint8_t {0});
    JsonToVariantOptions options = JsonToVariantOptions::current_config();
    options.throw_on_invalid_json = true;
    JsonStringToVariantEncoder encoder(options);
    const StringRef null_json("null", 4);
    for (size_t row = 0; row < strings.size(); ++row) {
        if (input_null_map != nullptr && input_null_map[row] != 0) {
            encoder.add_json(null_json);
            result_nulls->get_data()[row] = 1;
            continue;
        }

        const StringRef input = strings.get_data_at(row);
        Status status;
        std::string json_text;
        if (input.size == 0) {
            status = Status::InvalidArgument("JSON input is empty");
        } else if (input_is_jsonb) {
            const JsonbDocument* document = nullptr;
            status = JsonbDocument::checkAndCreateDocument(input.data, input.size, &document);
            if (status.ok()) {
                try {
                    json_text = JsonbToJson {}.to_json_string(document->getValue());
                    status = encoder.try_add_json({json_text.data(), json_text.size()});
                } catch (const Exception& exception) {
                    status = exception.to_status();
                }
            }
        } else {
            status = encoder.try_add_json(input);
        }
        if (status.ok()) {
            continue;
        }
        if (!error_to_null) {
            return Status::InvalidArgument("Parse json document failed at row {}, error: {}", row,
                                           status.to_string());
        }
        encoder.add_json(null_json);
        result_nulls->get_data()[row] = 1;
    ...
    *result = VariantParseResult {.values = std::move(values), .nulls = std::move(result_nulls)};
    return Status::OK();
}
...
public:
    ...
    Status execute_impl(FunctionContext* context, Block& block, const ColumnNumbers& arguments,
        ...
        const DataTypePtr input_type = remove_nullable(block.get_by_position(arguments[0]).type);
        const bool input_is_jsonb = input_type->get_primitive_type() == PrimitiveType::TYPE_JSONB;
        ...
        DORIS_CHECK(dynamic_cast<const DataTypeVariant*>(result_type) != nullptr);
        VariantParseResult parsed;
        RETURN_IF_ERROR(parse_json_strings_to_variant(
                *strings, input_nulls == nullptr ? nullptr : input_nulls->data(), input_is_jsonb,
                ERROR_TO_NULL, &parsed));

        ColumnPtr output;
        if (result_is_nullable) {
            output = ColumnNullable::create(std::move(parsed.values), std::move(parsed.nulls));
        } else {
            output = std::move(parsed.values);
        }
...
void register_function_variant_parse(SimpleFunctionFactory& factory) {
    ...
    factory.register_alias(FunctionTryParseToVariant::name, "parse_to_variant_error_to_null");
}
