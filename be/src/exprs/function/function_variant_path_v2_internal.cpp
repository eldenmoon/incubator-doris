...
bool VariantPathV2BatchReader::find_at(size_t row, VariantRef* const output) {
    ...
    for (size_t position = 0; position < _path.size(); ++position) {
        if (_path.kind_at(position) == VariantElementV2PathSegment::Kind::OBJECT_KEY) {
            ...
            const uint32_t element_count = current.num_elements();
            uint32_t resolved_index = 0;
            if (requested_index >= 0) {
                if (static_cast<uint64_t>(requested_index) >= element_count) {
                    return false;
                }
                resolved_index = static_cast<uint32_t>(requested_index);
            } else {
                const uint64_t from_end = static_cast<uint64_t>(-(requested_index + 1)) + 1;
                if (from_end > element_count) {
                    return false;
                }
                resolved_index = element_count - static_cast<uint32_t>(from_end);
            }
            current = current.array_at(resolved_index);
        }
    }
    *output = current;
    return true;
