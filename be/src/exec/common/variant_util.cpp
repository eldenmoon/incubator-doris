...
bool needs_decimal256_storage_encoding(const ColumnWithTypeAndName& arg,
                                       const DataTypePtr& target_type) {
    if (target_type->get_primitive_type() != TYPE_VARIANT) {
        return false;
    }
    const DataTypePtr source_type = remove_nullable(arg.type);
    return source_type->get_primitive_type() == TYPE_DECIMAL256 &&
           source_type->get_precision() > 38;
}

Status encode_decimal256_storage_leaf(const ColumnWithTypeAndName& arg,
                                      const DataTypePtr& target_type, ColumnPtr* result) {
    ColumnPtr materialized = arg.column->convert_to_full_column_if_const();
    const DataTypePtr source_type = remove_nullable(arg.type);
    const IColumn* nested = materialized.get();
    CastWrapper::variant_v2_internal::ForcedNulls source_nulls;
    if (arg.type->is_nullable()) {
        const auto* nullable = check_and_get_column<ColumnNullable>(materialized.get());
        if (nullable == nullptr) {
            return Status::InvalidArgument("Nullable Variant scan source {} has column type {}",
                                           arg.type->get_name(), materialized->get_name());
        }
        nested = &nullable->get_nested_column();
        source_nulls = nullable->get_null_map_data();
    }

    const auto* decimals = check_and_get_column<ColumnDecimal256>(nested);
    if (decimals == nullptr) {
        return Status::InvalidArgument("Decimal256 Variant scan source {} has column type {}",
                                       arg.type->get_name(), nested->get_name());
    }
    const uint32_t scale = source_type->get_scale();
    VariantBatchBuilder builder({.rows = materialized->size(), .nodes = materialized->size()});
    for (size_t row = 0; row < materialized->size(); ++row) {
        auto output = builder.begin_row();
        if (!source_nulls.empty() && source_nulls[row] != 0) {
            output.add_null();
        } else {
            const std::string text = decimals->get_data()[row].to_string(scale);
    ...
    auto outer_nulls = ColumnUInt8::create(materialized->size(), 0);
    if (!source_nulls.empty()) {
        std::ranges::copy(source_nulls, outer_nulls->get_data().begin());
    }
    *result = ColumnNullable::create(std::move(variant), std::move(outer_nulls));
    return Status::OK();
}
...
Status cast_column(const ColumnWithTypeAndName& arg, const DataTypePtr& type, ColumnPtr* result) {
    if (type->get_primitive_type() == TYPE_VARIANT) {
        // Parquet Variant decimals stop at precision 38. A wider materialized storage leaf is
        // therefore encoded as its exact decimal string without widening public CAST support.
        if (needs_decimal256_storage_encoding(arg, type)) {
            return encode_decimal256_storage_leaf(arg, type, result);
        }
        // The nullable CAST path passes source nulls into the Variant V2 kernel. When a nullable
        // source is requested as non-nullable, remove only the result's outer SQL-null wrapper so
        // non-Variant inputs retain those nulls in their encoded value.
        const bool remove_outer_nullable = arg.type->is_nullable() && !type->is_nullable();
        DataTypePtr cast_type = remove_outer_nullable ? make_nullable(type) : type;
        ColumnsWithTypeAndName variant_arguments {arg, {nullptr, cast_type, cast_type->get_name()}};
        auto function = SimpleFunctionFactory::instance().get_function("CAST", variant_arguments,
                                                                       cast_type);
        if (!function) {
        ...
        Block tmp_block {variant_arguments};
        auto result_column = cast_set<uint32_t>(tmp_block.columns());
        RuntimeState state;
        auto ctx = FunctionContext::create_context(&state, {}, {});
        ctx->set_string_as_jsonb_string(true);
        ctx->set_jsonb_string_as_string(true);
        tmp_block.insert({nullptr, cast_type, arg.name});
        RETURN_IF_ERROR(
        ...
        *result = remove_outer_nullable ? remove_nullable(converted) : std::move(converted);
        return Status::OK();
    }
...
bool generate_sub_column_info(const TabletSchema& schema, int32_t col_unique_id,
                              ...
                              PatternTypePB* matched_pattern_type) {
    ...
    for (const auto& sub_column : sub_columns) {
        ...
        case PatternTypePB::MATCH_NAME: {
            if (strcmp(pattern, path.c_str()) == 0) {
                ...
                if (matched_pattern_type != nullptr) {
                    *matched_pattern_type = sub_column->pattern_type();
                }
                return true;
            }
            break;
