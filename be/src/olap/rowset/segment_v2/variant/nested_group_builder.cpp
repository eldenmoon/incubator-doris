...
Status NestedGroupBuilder::_process_jsonb_value(const doris::JsonbValue* value,
    ...
    if (value->isObject()) {
        ...
        for (auto it = obj->begin(); it != obj->end(); ++it) {
            ...
            vectorized::PathInData next = vectorized::PathInData::append(current_path, key);
            RETURN_IF_ERROR(
                    _process_jsonb_value(it->value(), next, nested_groups, row_idx, depth + 1));
        }
        return Status::OK();
    }
    ...
    if (value->isArray()) {
        ...
        // For top-level arrays (current_path is empty), use special kRootNestedGroupPath path marker.
...
bool NestedGroupBuilder::_is_array_of_objects(const doris::JsonbValue* arr_value) const {
    ...
    bool has_non_null_elem = false;
    for (int i = 0; i < n; ++i) {
        const auto* elem = arr->get(i);
        if (!elem || elem->isNull()) {
            continue;
        }
        has_non_null_elem = true;
        if (!elem->isObject()) {
            return false;
        }
    }
    return has_non_null_elem;
}
...
Status NestedGroupBuilder::_process_array_of_objects(const doris::JsonbValue* arr_value,
    ...
    OffsetManager::pad_group(group, parent_row_idx, OffsetManager::PadMode::ElementExclusive);
    ...
    for (auto& [_, sub] : group.children) {
        if (sub.size() < group.current_flat_size) {
            sub.insert_many_defaults(group.current_flat_size - sub.size());
        }
    }
    for (auto& [_, ng] : group.nested_groups) {
        OffsetManager::pad_group(*ng, group.current_flat_size,
                                 OffsetManager::PadMode::ElementExclusive);
    }

    return Status::OK();
}

Status NestedGroupBuilder::_process_object_as_paths(const doris::JsonbValue* obj_value,
    ...
    for (const auto& kv : *obj) {
        ...
        vectorized::PathInData next_prefix = vectorized::PathInData::append(current_prefix, key);
        ...
        if (v->isObject()) {
            RETURN_IF_ERROR(_process_object_field(v, next_prefix, group, element_flat_idx, depth));
        } else if (v->isArray() && _is_array_of_objects(v)) {
            RETURN_IF_ERROR(
                    _process_nested_array_field(v, next_prefix, group, element_flat_idx, depth));
        } else {
...
Status NestedGroupBuilder::_process_object_field(const doris::JsonbValue* obj_value,
                                                 const vectorized::PathInData& next_prefix,
                                                 NestedGroup& group, size_t element_flat_idx,
                                                 size_t depth) {
    // Recursively flatten object fields into dotted paths.
    return _process_object_as_paths(obj_value, next_prefix, group, element_flat_idx, depth + 1);
}

Status NestedGroupBuilder::_process_nested_array_field(const doris::JsonbValue* arr_value,
                                                       const vectorized::PathInData& next_prefix,
                                                       NestedGroup& group, size_t element_flat_idx,
                                                       size_t depth) {
    ...
    OffsetManager::pad_group(*ng, element_flat_idx, OffsetManager::PadMode::ElementExclusive);
...
Status NestedGroupBuilder::_process_scalar_field(const doris::JsonbValue* value,
                                                 const vectorized::PathInData& next_prefix,
                                                 NestedGroup& group, size_t element_flat_idx) {
...
Status NestedGroupBuilder::_jsonb_to_field(const doris::JsonbValue* value,
    ...
    case doris::JsonbType::T_Int32:
    case doris::JsonbType::T_Int64:
        out = vectorized::Field::create_field<PrimitiveType::TYPE_BIGINT>(
                static_cast<int64_t>(value->int_val()));
        return Status::OK();
    case doris::JsonbType::T_Double:
        out = vectorized::Field::create_field<PrimitiveType::TYPE_DOUBLE>(
                value->unpack<doris::JsonbDoubleVal>()->val());
        return Status::OK();
    case doris::JsonbType::T_Float:
        out = vectorized::Field::create_field<PrimitiveType::TYPE_DOUBLE>(
                static_cast<double>(value->unpack<doris::JsonbFloatVal>()->val()));
        return Status::OK();
    case doris::JsonbType::T_String: {
        const auto* s = value->unpack<doris::JsonbStringVal>();
...
bool NestedGroupBuilder::_handle_conflict(NestedGroup& group, bool is_array_object) const {
    ...
    if (group.expected_type == NestedGroup::StructureType::UNKNOWN) {
        group.expected_type = is_array_object ? NestedGroup::StructureType::ARRAY
