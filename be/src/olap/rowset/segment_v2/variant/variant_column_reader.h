...
// Forward declaration - defined in nested_group_iterator.h

// Forward declaration
struct NestedGroupReader;
struct NestedGroupPathFilter;
class NestedOffsetsMappingIndexReader;
...
struct NestedGroupReader {
    ...
    std::shared_ptr<NestedOffsetsMappingIndexReader> offsets_mapping_index;
...
private:
    ...
    bool _try_build_nested_group_plan(ReadPlan* plan, const TabletColumn& target_col,
                                      const StorageReadOptions* opt, int32_t col_uid,
                                      const vectorized::PathInData& relative_path) const;
    static ReadPlan from_nested_group_plan(const NestedGroupReadPlanner::Plan& plan);
    ...
    std::optional<NestedGroupPathFilter> _build_nested_group_path_filter(
