...
struct NestedGroupWriter {
    ...
    std::shared_ptr<NestedOffsetsMappingIndexWriter> offsets_mapping_index_writer;
    std::unordered_map<std::string, ColumnWriterOptions> child_opts;
    std::unordered_map<std::string, std::vector<std::shared_ptr<TabletIndex>>> child_inverted_indexes;
};
