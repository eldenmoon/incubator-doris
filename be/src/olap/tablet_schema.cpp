...
std::size_t HashOfTabletColumn::operator()(const TabletColumn& column) const {
    ColumnPB column_pb;
    column.to_schema_pb(&column_pb);
    std::string serailized_key = column_pb.SerializeAsString();
    return HashUtil::hash64(serailized_key.data(), serailized_key.size(), 0);
    // size_t seed = 0;
    // seed = HashUtil::hash64(&column._unique_id, sizeof(column._unique_id), seed);
    // seed = HashUtil::hash64(column._col_name.data(), column._col_name.size(), seed);
    // seed = HashUtil::hash64(column._col_name_lower_case.data(), column._col_name_lower_case.size(), seed);
    // seed = HashUtil::hash64(&column._type, sizeof(column._type), seed);
    // seed = HashUtil::hash64(&column._is_key, sizeof(column._is_key), seed);
    // seed = HashUtil::hash64(&column._aggregation, sizeof(column._aggregation), seed);
    // seed = HashUtil::hash64(column._aggregation_name.data(), column._aggregation_name.size(), seed);
    // seed = HashUtil::hash64(&column._is_nullable, sizeof(column._is_nullable), seed);
    // seed = HashUtil::hash64(&column._has_default_value, sizeof(column._has_default_value), seed);
    // seed = HashUtil::hash64(column._default_value.data(), column._default_value.size(), seed);
    // seed = HashUtil::hash64(&column._is_decimal, sizeof(column._is_decimal), seed);
    // seed = HashUtil::hash64(&column._precision, sizeof(column._precision), seed);
    // seed = HashUtil::hash64(&column._frac, sizeof(column._frac), seed);
    // seed = HashUtil::hash64(&column._length, sizeof(column._length), seed);
    // seed = HashUtil::hash64(&column._is_bf_column, sizeof(column._is_bf_column), seed);
    // seed = HashUtil::hash64(&column._has_bitmap_index, sizeof(column._has_bitmap_index), seed);
    // seed = HashUtil::hash64(&column._parent_col_unique_id, sizeof(column._parent_col_unique_id), seed);
    // for (size_t i = 0; i < column._sub_column_count; ++i) {
    //     seed ^= HashOfTabletColumn::operator()(column.get_sub_column(i));
    // }
    // seed = HashUtil::hash64(&column._sub_column_count, sizeof(column._sub_column_count), seed);
    // seed = HashUtil::hash64(&column._result_is_nullable, sizeof(column._result_is_nullable), seed);
    // seed ^= vectorized::PathInData::Hash{}(column._column_path);
    // // No need to caculate hash of _sparse_cols only _num_sparse_columns needed
    // seed = HashUtil::hash64(&column._num_sparse_columns, sizeof(column._num_sparse_columns), seed);
}
...
void TabletSchema::relace_column(size_t pos, TabletColumn new_col) {
...
Status TabletSchema::have_column(const std::string& field_name) const {
    if (!_field_name_to_index.count(StringRef(field_name))) {
...
const TabletColumn& TabletSchema::column(const std::string& field_name) const {
    DCHECK(_field_name_to_index.count(StringRef(field_name)) != 0)
