    ...
    for (size_t i = 0; i < _row_read_ctxs.size(); ++i) {
        if (_row_read_ctxs[i]._cached_row_data.valid()) {
            vectorized::JsonbSerializeUtil::jsonb_to_block(
                    ...
                    *_result_block, _reusable->get_data_type_serdes());
            continue;
        }
        ...
        vectorized::JsonbSerializeUtil::jsonb_to_block(
                *_reusable->tuple_desc(), value.data(), value.size(),
                _reusable->get_col_uid_to_idx(), *_result_block, _reusable->get_data_type_serdes());
    }
    return Status::OK();
}
