...
Status make_variant_shredder_options(const TabletSchema& tablet_schema,
    ...
    VariantShredderOptions result {
            ...
            .logical_root_path = logical_root_path,
    ...
    // A non-empty logical root identifies an already-extracted leaf writer. That writer owns one
    // physical target and must materialize the leaf instead of applying the parent root's top-N or
    // sparse-bucket policy.
    if (!result.logical_root_path.empty()) {
        result.max_subcolumns_count = 0;
        result.typed_paths_to_sparse = false;
        result.sparse_bucket_count = 1;
    }
