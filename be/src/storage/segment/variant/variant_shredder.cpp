...
struct VariantShredder::Impl {
    enum class PathMatch : uint8_t { UNKNOWN, NO, YES };
    ...
    Status validate_doc_path(uint32_t path_id) const {
        ...
        for (const PathInData::Part& part : parts) {
            if (part.key.empty()) {
                return Status::Corruption("Variant doc path has an empty part");
            }
        }
        return Status::OK();
    }

    bool is_exact_typed_string_path(uint32_t path_id) {
        if (typed_string_paths.size() <= path_id) {
            typed_string_paths.resize(path_id + 1, PathMatch::UNKNOWN);
        }
        PathMatch& cached = typed_string_paths[path_id];
        if (cached != PathMatch::UNKNOWN) {
            return cached == PathMatch::YES;
        }
        cached = PathMatch::NO;
        ...
        PatternTypePB matched_pattern_type = PatternTypePB::MATCH_NAME_GLOB;
        if (!variant_util::generate_sub_column_info(
                    *options.tablet_schema, options.parent_column_unique_id,
                    logical_path(path_id).get_path(), &info, &matched_pattern_type) ||
            matched_pattern_type != PatternTypePB::MATCH_NAME) {
            return false;
        }
        if (DataTypeFactory::instance().create_data_type(info.column)->get_primitive_type() ==
            TYPE_STRING) {
            cached = PathMatch::YES;
            return true;
        }
        return false;
    }

    bool is_fixed_typed_path(uint32_t path_id) {
        if (fixed_typed_paths.size() <= path_id) {
            fixed_typed_paths.resize(path_id + 1, PathMatch::UNKNOWN);
        }
        PathMatch& cached = fixed_typed_paths[path_id];
        if (cached != PathMatch::UNKNOWN) {
            return cached == PathMatch::YES;
        }
        cached = PathMatch::NO;
        if (options.tablet_schema == nullptr || options.typed_paths_to_sparse) {
            return false;
        }
        TabletSchema::SubColumnInfo info;
        if (variant_util::generate_sub_column_info(*options.tablet_schema,
                                                   options.parent_column_unique_id,
                                                   logical_path(path_id).get_path(), &info)) {
            cached = PathMatch::YES;
            return true;
        }
        return false;
    ...
    Status visit(VariantRef value, uint32_t metadata_plan_id, uint32_t path_id, size_t row) {
        if (value.is_null()) {
            // Explicit JSON null still establishes a fixed typed path for this segment. Retain an
            // all-null builder so its physical column and indexes are finalized consistently.
            if (is_fixed_typed_path(path_id)) {
                get_or_create_builder(path_id);
            }
            return Status::OK();
        }
        ...
        if (is_exact_typed_string_path(path_id)) {
            return get_or_create_builder(path_id)->append(value, row);
        }
    ...
    Status append_root(VariantRef value, bool outer_null) {
        // V1 reconstructs objects exclusively from shredded paths, so JSON-null and unresolved
        // leaves remain absent. Keep the root only for scalar and array values.
        if (outer_null || value.basic_type() == VariantBasicType::OBJECT) {
    ...
    void publish_root_and_materialized(const VariantPathSelection& selection,
                                       const DorisVector<VariantPathSelectionCandidate>& candidates,
                                       const DorisVector<VariantPathBuilder*>& candidate_builders,
                                       VariantShreddedColumns* result) {
        result->num_rows = rows;
        result->root_type = make_nullable(std::make_shared<DataTypeJsonb>());
        result->root_jsonb = ColumnNullable::create(std::move(root_values), std::move(root_nulls));
        ...
        for (size_t selected : selection.materialized) {
            ...
            PathInData publication_path =
                    options.physical_layout == VariantShredderPhysicalLayout::DOC
                            ...
                            : raw_path;
            result->materialized.push_back({.path = std::move(publication_path),
                                            .type = builder.type(),
                                            .column = builder.column(),
                                            .non_null_rows = builder.non_null_rows(),
                                            .is_typed_path = candidates[selected].is_typed_path});
            result->statistics.subcolumns_non_null_size[builder.path().get_path()] =
                    builder.non_null_rows();
        }
    }
    ...
    void select_sparse_statistics(DorisVector<SparsePlan>* sparse_plan) const {
        ...
        for (size_t row = 0; row < rows; ++row) {
            for (SparsePlan& plan : *sparse_plan) {
                if (plan.encountered || plan.builder->is_null_at(row)) {
                    continue;
                }
                plan.encountered = true;
                if (tracked_paths_per_bucket[plan.bucket] <
                    options.max_sparse_column_statistics_size) {
    ...
    Status append_binary_rows(const DorisVector<BinaryPlan>& binary_plan,
        ...
        for (size_t row = 0; row < rows; ++row) {
            for (const BinaryPlan& plan : binary_plan) {
                if (plan.builder->is_null_at(row)) {
                    continue;
                }
                ...
                RETURN_IF_ERROR(plan.builder->write_sparse_cell(row, &values.get_chars()));
    ...
    Status publish_sparse(const VariantPathSelection& selection,
        ...
        for (uint32_t bucket = 0; bucket < options.sparse_bucket_count; ++bucket) {
            ...
            sparse_owners.push_back(std::move(map));
        }
    ...
    Status build_doc_plan(const DorisVector<VariantPathBuilder*>& candidate_builders,
        ...
        std::sort(plan.begin(), plan.end(), [](const DocPlan& left, const DocPlan& right) {
    ...
    DorisVector<PathMatch> typed_string_paths;
    DorisVector<PathMatch> fixed_typed_paths;
    ColumnString::MutablePtr root_values = ColumnString::create();
    ColumnUInt8::MutablePtr root_nulls = ColumnUInt8::create();
...
Status VariantShredder::append(const ColumnVariantV2::ReadView& view, size_t begin, size_t length,
    ...
    try {
        ...
        const size_t completed_rows = _impl->rows + length;
        for (size_t offset = 0; offset < length; ++offset) {
            const bool outer_null = !outer_nulls.empty() && outer_nulls[offset] != 0;
            if (outer_null) {
                Status status = _impl->append_root({}, true);
                if (!status.ok()) {
                    return _impl->fail(std::move(status));
                }
                ++_impl->rows;
            ...
            Status status = _impl->append_root(value, false);
            if (!status.ok()) {
                return _impl->fail(std::move(status));
            }
            if (value.basic_type() == VariantBasicType::OBJECT) {
        ...
        Status status = _impl->complete_builder_rows(completed_rows);
        if (!status.ok()) {
            return _impl->fail(std::move(status));
        }
        return Status::OK();
