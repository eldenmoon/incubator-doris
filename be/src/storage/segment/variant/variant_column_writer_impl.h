...
// - Append the shredder's serialized sparse bucket maps to storage writers
...
// - Single mode appends the only shredder bucket and writes its statistics to meta.
// - Bucket mode appends the N pre-sharded buckets and writes each bucket's statistics to meta.
...
private:
    ...
    Status _write_v2_root(const VariantShreddedColumns& shredded, size_t num_rows, int& column_id);
    Status _write_v2_materialized(const VariantShreddedColumns& shredded,
                                  OlapBlockDataConvertor* converter, size_t num_rows,
                                  int& column_id);
    Status _write_v2_binary(const VariantShreddedColumns& shredded,
                            OlapBlockDataConvertor* converter, size_t num_rows, int& column_id);
    doris::ColumnVariantV2::MutablePtr _v2_column;
