...
constexpr uint32_t INVALID_PATH_ID = std::numeric_limits<uint32_t>::max();
...
void append_decimal(const ValueScratch& value, const DataTypePtr& target_type, IColumn* target) {
    ...
    __int128 converted = 0;
    if (!rescale_decimal(source_value, source_scale, target_type->get_scale(), &converted)) {
        throw Exception(ErrorCode::INVALID_ARGUMENT,
...
struct VariantPathBuilder::Impl {
    explicit Impl(PathInData path_, size_t prefix_rows_)
            : path(std::move(path_)),
              pending_prefix_rows(prefix_rows_),
    ...
    Status initialize(const DataTypePtr& initial_type) {
        ...
        if (pending_prefix_rows != 0) {
            column->insert_many_defaults(pending_prefix_rows);
            pending_prefix_rows = 0;
        }
        return Status::OK();
    }

    Status promote(const DataTypePtr& target_type) {
        ...
        if (target->get_primitive_type() == TYPE_STRING &&
            (type->get_primitive_type() == TYPE_ARRAY ||
             type->get_primitive_type() == TYPE_JSONB)) {
...
Status VariantPathBuilder::append(VariantRef value, size_t row) {
    try {
        ...
        if (!_impl->column) {
            ...
            RETURN_IF_ERROR(_impl->promote(common_type));
        }
        ...
        try {
            ...
            RETURN_IF_ERROR(_impl->promote(std::make_shared<DataTypeJsonb>()));
            auto& nullable = assert_cast<ColumnNullable&>(*_impl->column);
            append_value(scratch, _impl->type, &nullable.get_nested_column());
            nullable.get_null_map_data().push_back(0);
        }
...
Status VariantPathBuilder::complete_rows(size_t rows) {
    ...
    const size_t missing = rows - _impl->logical_rows;
    if (missing == 0) {
        return Status::OK();
    }
    if (_impl->column) {
        _impl->column->insert_many_defaults(missing);
    } else {
        _impl->pending_prefix_rows += missing;
    }
...
Status VariantPathBuilder::convert_to(const DataTypePtr& storage_type) {
    ...
    return _impl->promote(storage_type);
}
...
VariantPathSelection select_variant_paths(std::span<const VariantPathSelectionCandidate> candidates,
    ...
    std::sort(dynamic.begin(), dynamic.end(), [&](const auto& left, const auto& right) {
    ...
    std::sort(result.materialized.begin(), result.materialized.end(), by_path);
    std::sort(result.sparse.begin(), result.sparse.end(), by_path);
    return result;
}
...
struct VariantMetadataPathPlan::Impl {
    ...
    struct PathPlan {
        PathInData path;
        DorisVector<DorisVector<uint32_t>> transitions;
    };
    ...
    std::unordered_map<std::string, PathInData> display_paths;
    DorisVector<PathPlan> paths = {PathPlan {.path = PathInData(), .transitions = {}}};
};

// NOLINTNEXTLINE(modernize-use-equals-default): construction must allocate the opaque Impl.
...
Status VariantMetadataPathPlan::resolve_child(uint32_t parent_path_id, uint32_t metadata_plan_id,
    ...
    auto& parent = _impl->paths[parent_path_id];
    if (parent.transitions.size() <= metadata_plan_id) {
        parent.transitions.resize(metadata_plan_id + 1);
    }
    auto& transitions = parent.transitions[metadata_plan_id];
    if (transitions.empty()) {
        transitions.resize(metadata.keys.size(), INVALID_PATH_ID);
    }
    if (transitions[field_id] != INVALID_PATH_ID) {
        *child_path_id = transitions[field_id];
        return Status::OK();
    }
    ...
    if (const auto display = _impl->display_paths.find(child.get_path());
        display != _impl->display_paths.end() && display->second != child) {
        return Status::InvalidArgument(
                "Variant paths with distinct structures collide at dotted path {}",
                child.get_path());
    }
    ...
    if (const auto found = _impl->path_ids.find(child); found != _impl->path_ids.end()) {
        ...
        _impl->display_paths.emplace(child.get_path(), child);
        _impl->paths.push_back(Impl::PathPlan {.path = std::move(child), .transitions = {}});
    }
    ...
    stable_parent.transitions[metadata_plan_id][field_id] = id;
    *child_path_id = id;
    return Status::OK();
}
