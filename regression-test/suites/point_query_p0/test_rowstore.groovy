...
suite("test_rowstore", "p0") {
    ...
    qt_sql """
        select length(__DORIS_ROW_STORE_COL__group1), length(__DORIS_ROW_STORE_COL__group2) from table_with_column_group order by k1;
    """
    ...
    """
    qt_sql """
        select length(__DORIS_ROW_STORE_COL__group2) from table_with_column_group2 order by k1;
    """
    ...
    """
    qt_sql """
        select length(__DORIS_ROW_STORE_COL__group1) from table_with_column_group3 order by k1;
    """
    ...
    sql """
            ...
            PROPERTIES ( "replication_num" = "1",
                    ...
                    "light_schema_change" = "true",
                    "store_row_column" = "false",
                    "column_groups" = "group1:datetimev2_1,datetime_val1,datetime_val2,max_dwell_time"
            );
        """
    ...
    connect(user = user, password = password, url = prepare_url) {
        ...
        // group1
        def sql_str = "select v1, v2, v3 from table_with_column_group where k1 = ?"
        prep_sql sql_str, 1
        prep_sql sql_str, 2 
        prep_sql sql_str, 3
        // group 2
        sql_str = "select v2, v4 from table_with_column_group where k1 = ?"
        prep_sql sql_str, 1
        prep_sql sql_str, 2 
        prep_sql sql_str, 3
        // group 1
        sql_str = "select v2, v3 from table_with_column_group where k1 = ?"
        prep_sql sql_str, 3
    ...
    try {
        ...
        log.info("ecounter {}", e.getMessage());
        assertTrue(e.getMessage().contains("Not support column store"))
    }
