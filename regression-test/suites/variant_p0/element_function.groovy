...
suite("regression_test_variant_element_at", "p0")  {
    ...
    // (e.g. `parse_to_variant(text)['key']`) must not leak the surrounding JSON
    ...
    def scalar = sql """select parse_to_variant('{"wsn":"SRFSPXFDVY","uploadTimeValue":"2026-05-20 18:40:02"}')['wsn']"""
    assertEquals("SRFSPXFDVY", scalar[0][0])

    def sub = sql """select substring(parse_to_variant('{"uploadTimeValue":"2026-05-20 18:40:02"}')['uploadTimeValue'], 1, 10)"""
    assertEquals("2026-05-20", sub[0][0])

    // values containing escaped characters must be unescaped, not kept as raw JSON tokens
    def escaped = sql """select parse_to_variant('{"k":"a\\\\"b"}')['k']"""
    assertEquals("a\"b", escaped[0][0])

    // non-string scalars keep their existing JSON representation
    def num = sql """select parse_to_variant('{"n":49.98}')['n']"""
    ...
    def arr = sql """select cast(parse_to_variant('{"a":[1,2,3]}')['a'] as json)"""
    assertEquals("[1,2,3]", arr[0][0])

    def obj = sql """select sort_json_object_keys(json_extract(
            cast(parse_to_variant('{"o":{"name":"john"}}') as json), '\$.o'))"""
