...
suite("regression_test_variant_predefine_multi_var", "variant_type"){
    ...
    """
    sql """INSERT INTO ${table_name} SELECT *, parse_to_variant('{"k1":1, "k2": "hello world", "k3" : [1234], "k4" : 1.10000, "k5" : [[123]]}') FROM numbers("number" = "101")"""
    sql """INSERT INTO ${table_name} SELECT *, parse_to_variant('{"k7":123, "k8": "elden ring", "k9" : 1.1112, "k10" : [1.12], "k11" : ["moon"]}') FROM numbers("number" = "203") where number > 100"""
    sql """INSERT INTO ${table_name} SELECT *, parse_to_variant('{"k7":123, "k8": "elden ring", "k9" : 1.1112, "k10" : [1.12], "k11" : ["moon"]}') FROM numbers("number" = "411") where number > 200"""
    ...
    sql """INSERT INTO ${table_name} select k, v, parse_to_variant(cast(v as string)) from ${table_name}"""

    sql """alter table ${table_name} add column v3 variant<'k1' : int, 'k2' : string, 'k3' : array<int>, 'k4' : double> default null"""
    sql """INSERT INTO ${table_name} select k, v, parse_to_variant(cast(v as string)), parse_to_variant(cast(v as string)) from ${table_name}"""
    sql "alter table ${table_name} add column ss string default null"
    sql """INSERT INTO ${table_name} select k, v, parse_to_variant(cast(v as string)), parse_to_variant(cast(v as string)), cast(v as string) from ${table_name}"""
    ...
    for (int i = 0; i < 20; i++) {
        sql """insert into ${table_name} values (1, parse_to_variant('{"a" : 1}'), parse_to_variant('{"a" : 1}'), parse_to_variant('{"a" : 1}'), '{"a" : 1}', parse_to_variant('{"a" : 1}'))"""
    }
    trigger_and_wait_compaction(table_name, "cumulative", 1800)
}
