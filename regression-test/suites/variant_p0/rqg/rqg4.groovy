...
suite("rqg4", "p0,nonConcurrent") {
    setFeConfigTemporary([enable_variant_v2: false]) {
        ...
        String sqlText = new File(context.dataPath, "rqg4.sql").text
        sqlText = sqlText.replace("parse_to_variant_if_v2(", "(")
                .replace("variant_decimal_type_if_v2", "decimal(76, 56)")
        List<String> sqls = SqlUtils.splitAndGetNonEmptySql(sqlText)
        String exceptions = ""
        sqls.eachWithIndex { String statement, int index ->
            String tag = index == 0 ? "rqg4" : "rqg4_${index + 1}"
            try {
