...
suite("compaction_variant_doc_value", "p0") {
    ...
    try {
        ...
        for (int i = 0; i < key_types.size(); i++) {
            ...
            qt_sql_3 "select k, cast(v['a'] as string), cast(v['b'] as string) from  ${tableName} where  length(cast(v['b'] as string)) > 4 order  by k"
            ...
            qt_sql_33 "select k, cast(v['a'] as string), cast(v['b'] as string) from  ${tableName} where  length(cast(v['b'] as string)) > 4 order  by k"
        }
