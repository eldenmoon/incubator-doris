...
SELECT sum(length(cast(v["payload"]["pull_request"]["base"]["repo"]["html_url"] as string))) FROM ghdata;
...
select k, json_extract(cast(v as jsonb), '$.repo') from ghdata WHERE cast(v["type"] as string) = 'WatchEvent'  order by k limit 10;
...
select k, cast(v["id"] as bigint), cast(v["type"] as string), cast(v["repo"]["name"] as string) from ghdata WHERE cast(v["type"] as string) = 'WatchEvent'  order by k limit 10;
SELECT cast(v["payload"]["pusher_type"] as string) FROM ghdata where v["payload"]["pusher_type"] is not null ORDER BY k LIMIT 10;
