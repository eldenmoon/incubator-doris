...
// The on-disk format version of the VARIANT values index is stamped by the FE; only the stamped
// version is accepted back (so SHOW CREATE TABLE can be replayed). The legacy spelling
// `variant_index_mode` is accepted and normalized to `variant_index_scope`, and recursive scalar
// leaves are always indexed.
suite("test_variant_all_values_legacy_format", "p0,nonConcurrent") {
    setFeConfigTemporary([enable_variant_v2: true]) {
        ...
        test {
            ...
            exception "unsupported variant_root_format_version: 1"
        }
        sql """CREATE TABLE variant_all_values_legacy_format (
            id BIGINT NOT NULL,
            v VARIANT,
