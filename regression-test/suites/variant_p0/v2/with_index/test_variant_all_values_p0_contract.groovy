...
suite("test_variant_all_values_p0_contract", "p0,nonConcurrent") {
    setFeConfigTemporary([enable_variant_v2: true]) {
        ...
        [variant_all_values_p0_contract: 16, variant_all_values_p0_sparse: 1].each {
            ...
            try {
                // 13 rows, 'leafvalue' is a leaf of rows 1 and 2 only.
                GetDebugPoint().enableDebugPointForAllBEs(checkpoint, [filtered_rows: "11"])
            ...
            def predicates = [
                ...
                root_eq: "CAST(payload AS STRING) = 'abc'",
                root_in: "CAST(payload AS STRING) IN ('abc', 'zzz')"
            ]
            predicates.each { name, predicate ->
                ...
                def indexed
                // Strings longer than ignore_above have no exact term, so their predicates fall
                // back to the scan, and so does whole-document equality, which compares the JSON
                // text of the value that no leaf term bounds. Numbers are typed terms that
                // ignore_above does not apply to; `n` is the hottest path and stays materialized
                // in both layouts, so its predicates are answered from the index.
                def mustFallBack = name in ['long_eq', 'mixed_in', 'root_eq', 'root_in']
                try {
                    if (mustFallBack) {
